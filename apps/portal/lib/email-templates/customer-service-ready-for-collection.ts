// Vehicle Service parallel of customer-ready-for-collection.ts — kept separate so Job Card's own
// email is never touched (project standing rule).
import { renderEmailLayout, escapeHtml } from './layout';
import { ordinal } from '../utils/ordinal';

export type CustomerServiceReadyForCollectionEmailOptions = {
  customerName: string;
 ...
 */
export function renderCustomerServiceReadyForCollectionEmail(opts: CustomerServiceReadyForCollectionEmailOptions): string {
  const { customerName, serviceNumber, vehicleDescription, dueDate, reminderNumber, dashboardUrl, logoUrl, companyName, branchName } = opts;
  ...
  const bodyHtml = `
    ...
    <p style="margin: 0 0 16px 0;">
      ...
      ${escapeHtml(vehicleDescription)} (Vehicle Service ${escapeHtml(serviceNumber)}) is ready for collection. You're
  ...
  return renderEmailLayout({
    previewText: `Ready for collection — Vehicle Service ${serviceNumber}, expected collection by ${dueDate}.`,
    companyName,

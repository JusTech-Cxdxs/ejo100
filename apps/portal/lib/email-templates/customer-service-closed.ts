// Vehicle Service parallel of customer-job-card-closed.ts — kept separate so Job Card's own
// email is never touched (project standing rule).
import { renderEmailLayout, escapeHtml } from './layout';

export type CustomerServiceClosedEmailOptions = {
  customerName: string;
 ...
 */
export function renderCustomerServiceClosedEmail(opts: CustomerServiceClosedEmailOptions): string {
  ...
  const bodyHtml = `
    ...
    <p style="margin: 0 0 16px 0;">
      Your ${escapeHtml(vehicleDescription)} (Vehicle Service ${escapeHtml(serviceNumber)}) has now been closed — the work
      and payment on this service are fully settled. Thank you for choosing ${escapeHtml(companyName)}.
    </p>
  `;

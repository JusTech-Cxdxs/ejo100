...
async function main() {
  ...
  //
  // Uses find-then-update-or-create (rather than "only create if entirely
  // missing") so that re-running this seed always keeps the password hash
  // in sync — this matters concretely: the field was previously named
  // `passwordHash` in the schema, which didn't match what Better Auth's
  // Prisma adapter actually reads (`password`), so the very first seed run
  // silently wrote the hash to a field Better Auth never looked at. After
  // correcting the schema field name, simply re-running the old
  // "create only if missing" logic would have skipped this row entirely
  // (it already existed) and never populated the now-correctly-named
  // field. This version fixes the row in place instead.
